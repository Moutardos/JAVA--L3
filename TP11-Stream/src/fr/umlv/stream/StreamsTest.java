...
public class StreamsTest {
	...
	public static long countNonEmpty(String[] array){
		...
		// Je n'ai pas trouvé une fonction isNotEmpty pour String
	...
	public static List<String> capitalizeList(List<String> list){
		return list.stream().filter(s -> s == s.toUpperCase(Locale.ROOT)).toList();
	}
	...
	public static Map<Character, Set<String>> mapByFirstCharacterSet(List<String> list){
		...
		var hashMapFirstSet = new HashMap<Character, Set<String>>();
		mapByFirstCharacter(list).forEach((key,value) -> value.forEach(s ->
				hashMapFirstSet.computeIfAbsent(key, k-> new HashSet<String>()).add(s)));
		return hashMapFirstSet;
	}
	...
	public static Map<Character, Long> countByFirstCharacter(List<String> list){
		var hashMapFirst = list.stream().collect(
	...
	public static List<String> words(String s){
		return Arrays.stream(s.split(" ")).filter(string-> !string.isEmpty()).toList();
	}
	...
	public static Set<String> words(List<String> list){
		...
		list.stream().flatMap(s -> words(s).stream()).forEach(setAns::add);
		return setAns;
	}
	...
	public static Set<String> unpack(List<Optional<String>> list){
		Set<String> setAns = new HashSet<>();
		list.stream().flatMap(Optional::stream).forEach(setAns::add);
		return setAns;
	}
